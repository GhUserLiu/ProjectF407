...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
// 消抖参数：50ms @ 168MHz = 8,400,000 cycles（取稍大值确保可靠）
#define DEBOUNCE_CYCLES    (168000000 / 20)   // 50ms

static uint32_t last_interrupt_cycle = 0;

// 外部函数声明
extern uint32_t DWT_GetCycle(void);
extern void Gear_Shift(void);
/* USER CODE END 0 */
