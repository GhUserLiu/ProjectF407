...
**分组说明：** 本实验为团队任务，已完成分组，每组2-3人。

| 组号 | 角色 | 姓名 | 学号 | 主要职责 |
|------|------|------|------|----------|
| 第__组 | 组长 |  |  | 统筹协调、系统集成、报告整合 |
| 第__组 | 成员 |  |  | 硬件设计、引脚配置、调试 |
| 第__组 | 成员 |  |  | 软件编程、中断实现、文档整理 |
...
**烧录说明：** 本实验通过USB转串口工具（如CH340）配合ISP下载软件（如MCUISP、FlyMcu）进行固件烧录，无需ST-Link调试器。
...
1. 使用Makefile或IDE编译工程
2. 使用ST-Link将固件烧录到开发板
3. 按下KEY0按键，观察LED变化
